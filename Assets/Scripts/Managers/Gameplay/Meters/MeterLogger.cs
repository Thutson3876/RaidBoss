...
public static class MeterLogger
{
    ...
    public static async void LogMeters(bool battleWon, int difficulty, List<int> modifierIDs, int bossId, float battleDuration, Dictionary<string, float> damage, Dictionary<string, float> stagger, Dictionary<string, float> healing)
    {
        string meterToken = FetchMeterToken();

        string json = JsonConvert.SerializeObject(new { meterToken, battleWon, difficulty, bossId, modifierIDs, battleDuration, damage, stagger, healing });
